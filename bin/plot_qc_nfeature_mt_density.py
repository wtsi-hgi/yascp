...
def calculate_density(df_plot, facet_column):
    """Calculate the density for each category in the facet.

    Return an array that corresponds to the row of the df_plot.
    """
...
def plot_nfeature_mt_density(
    df_plot,
    output_file='plot_nfeature_mt_density',
    facet_column='none',
    color_var='density',
    density_contour=False
):
    """Plot plot_nfeature_mt_density to png.

    Parameters
    ----------
    df_plot : pandas.DataFrame
        DataFrame with the followig keys 'n_genes_by_counts',
...
def main():
    ...
    parser.add_argument(
        ...
        action='store',
        dest='of',
        default='plot_nfeature_mt_density',
    ...
    for facet in facets_to_plot:
        try:
            plot_nfeature_mt_density(
                df_plot=adata.obs,
                output_file='plot_nfeature_mt_density.facet={}-{}'.format(
                    facet,
                    options.of
                ),
                facet_column=facet,
            ...
            if 'cell_passes_qc' in adata.obs:
                plot_nfeature_mt_density(
                    df_plot=adata.obs,
                    output_file='plot_nfeature_mt_cellpassqc.facet={}-{}'.format(
                        facet,
                        options.of
                    ),
                    facet_column=facet,
