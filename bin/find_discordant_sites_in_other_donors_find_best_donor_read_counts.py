...
class VCF_Loader:
    ...
    def __init__(self, vcf_file, exclude_a2g, biallelic_only=True,
                        sparse=False, format_list=['GT']):
        self.vcf_file = vcf_file
        self.exclude_a2g = exclude_a2g
    ...
    def load_sample_mp(self,line,obs_ids,count,format_list):
        ...
        elif list_val[3] == 'A' and list_val[4] == 'G' and self.exclude_a2g:#remove A>G
            pass
        elif list_val[3] == 'T' and list_val[4] == 'C' and self.exclude_a2g:#also remove T>C
            pass
        else:
...
class Concordances:
    ...
    def read_concordance(self, expected_vars, cell_vars):
        '''
        ...
        cell_vars2_disc = cell_vars2[(cell_vars2['AD'] > 0) | (cell_vars2['OTH'] > 0)]
        cell_vars2_disc_sites = set(cell_vars2_disc['ids'])
        for s2 in cell_vars2_disc_sites:
            sdf = cell_vars2_disc[cell_vars2_disc['ids'] == s2]
            if not len(sdf) == 1:
                print('error, 1 variant expected')
                print(sdf)
        ...
        # cell_vars3_disc = cell_vars3[cell_vars3['OTH'] > 0]
        # cell_vars3_disc_sites = set(cell_vars3_disc['ids'])
        # for s3 in cell_vars3_disc_sites:
        #     sdf = cell_vars3_disc[cell_vars3_disc['ids'] == s3]
        #     if not len(sdf) == 1:
        #         print('error, 1 variant expected')
        #         print(sdf)
            # discordant_site_details['expect_het'][s3] = {}
            # discordant_site_details['expect_het'][s3]['OTH'] = sdf['OTH'].sum() 
        ...
        cell_vars4['non_ref_reads'] = cell_vars4['AD'] = cell_vars4['OTH']
        cell_vars4['ref_reads'] = cell_vars4['DP'] - cell_vars4['non_ref_reads']
        # DP + OTH - AD
        ...
        cell_vars4_disc = cell_vars4[(cell_vars4['ref_reads'] > 0) | (cell_vars4['OTH'] > 0)]
        cell_vars4_disc_sites = set(cell_vars4_disc['ids'])
        for s4 in cell_vars4_disc_sites:
            sdf = cell_vars4_disc[cell_vars4_disc['ids'] == s4]
            if not len(sdf) == 1:
                print('error, 1 variant expected')
                print(sdf)
        ...
        return total_sites, total_reads, discordant_reads, discordant_site_details
    ...
    def retrieve_concordant_discordant_sites(self,expected_vars_norm,cell_vars):            
        ...
        if len(cell_vars_norm) > 0:
            ...
            total_sites, total_reads, discordant_reads, discordant_site_details = self.read_concordance(expected_vars2, cell_vars2)

            return total_sites, true_discordant_count, total_concordant_sites, total_reads, discordant_reads, discordant_site_details, discordant_vars, concordant_vars
    ...
    def conc_table(self):
        ...
        for don_id in donor_list:
            ...
            #donor_gt_vars = donor_gt_vars[donor_gt_vars['vars'] != '0/0'] ref/ref now wanted too
    ...
    def concordance_table_production(self,expected_vars_norm,cell_vars,cell1,donor_gt_match, donor_gt_match_cohort, vars_per_donor_gt, donor_cohorts, count,all_donor_data):
        ...
        total_sites, true_discordant_count, total_concordant_sites, total_reads, discordant_reads, discordant_site_details, discordant_vars, concordant_vars = self.retrieve_concordant_discordant_sites(expected_vars_norm,cell_vars)
        ...
        for donor in vars_per_donor_gt:
            ...
            total_sites_otherDonor, true_discordant_count_otherDonor, total_concordant_sites_otherDonor, total_reads_otherDonor, discordant_reads_otherDonor, discordant_site_detail_otherDonor, discordant_vars_otherDonor, concordant_vars_otherDonor = self.retrieve_concordant_discordant_sites(expected_vars_norm_of_other_donor,cell_vars)
            ...
            if not donor == donor_gt_match:
                ...
                expected_hom_alts_in_other_donors_has_ref = expected_hom_alts_in_other_donors[mask2]
                if (len(expected_hom_alts_in_other_donors_has_ref) > 0):
                    has_ref_vars = list(expected_hom_alts_in_other_donors_has_ref['ids'])
                    for v in has_ref_vars:
        ...
        discordant_alt_expected_contamination = 0
        for v1 in discordant_site_details['expect_hom_alt'].keys():
            if discordant_site_details['expect_hom_alt'][v1]['found_in_other_donor'] == 'no':
                discordant_alt_expected_error = discordant_alt_expected_error + discordant_site_details['expect_hom_alt'][v1]['REF']
            else:
                discordant_alt_expected_contamination = discordant_alt_expected_contamination + discordant_site_details['expect_hom_alt'][v1]['REF']
...
def get_options():
    ...
    parser.add_argument('--exclude_a2g', action='store_true')
...
if __name__ == "__main__":
    ...
    exclude_a2g = options.exclude_a2g
    ...
    if options.debug:
        ...
        if (os.path.exists(gt_match_vcf)):
            loader2 = VCF_Loader(gt_match_vcf, exclude_a2g, biallelic_only=True,
                            sparse=False, format_list=['GT'])
        ...
        loader1 = VCF_Loader(cell_vcf, exclude_a2g, biallelic_only=True,
        ...
        loader3 = VCF_Loader(expected_vcf, exclude_a2g, biallelic_only=True,
                        sparse=False, format_list=['GT'])
