...
for vcf1 in all_vcfs2:
    ...
    os.makedirs(f"{PREFIX1}captured/filtered/{name}", exist_ok=True)
    os.makedirs(f"{PREFIX1}captured/unfiltered/{name}", exist_ok=True)

    if 'unfiltered' in vcf1:
        os.system(f'ln -s {vcf1} {PREFIX1}captured/unfiltered/{name}')
    else:
        os.system(f'ln -s {vcf1} {PREFIX1}captured/filtered/{name}')