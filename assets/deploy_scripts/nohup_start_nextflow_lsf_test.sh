...
export NXF_LOG_LEVEL=TRACE
export NXF_CLI_LOG=true
export NXF_CONSOLE_DUMP=true
export NXF_DEBUG=5
...
echo $RUN_ID | unbuffer nextflow run $SCRIPT_DIR/../.. -profile sanger,test $INPUT_FILE --nf_ci_loc $PWD -resume  2>&1 | \
    sed -r "s/\x1b\[[0-9;]*m//g" | \
    ts '[%Y-%m-%d %H:%M:%S]' | \
