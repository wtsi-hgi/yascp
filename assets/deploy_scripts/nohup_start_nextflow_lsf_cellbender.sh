...
echo $RUN_ID | nextflow run $SCRIPT_DIR/../.. -profile sanger -entry JUST_CELLBENDER $INPUT_FILE -c $SCRIPT_DIR/../../assets/deploy_scripts/input_setups/cellbender_profile.nf --nf_ci_loc $PWD -resume 2>&1 | \
    sed -r "s/\x1b\[[0-9;]*m//g" | \
    ts '[%Y-%m-%d %H:%M:%S]'  | \
