...
echo $RUN_ID | nextflow run $SCRIPT_DIR/../.. -profile sanger -entry JUST_RECLUSTER -c $SCRIPT_DIR/../../assets/deploy_scripts/input_setups/recluster_profile.nf $INPUT_FILE  --nf_ci_loc $PWD -resume 2>&1 | \
    sed -r "s/\x1b\[[0-9;]*m//g" | \
    ts '[%Y-%m-%d %H:%M:%S]'  | \
