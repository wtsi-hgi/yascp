...
process JOIN_CHROMOSOMES{
    ...
    script:
      ...
      """
        ...
        bcftools view no_prefix_pre_\${vcf_name}.vcf.gz -Oz -o no_prefix2_pre_\${vcf_name}.vcf.gz
    ...
    script:
      ...
      """
        ...
        if [ \$(cat fofn_vcfs.txt | wc -l) -gt 1 ]; then
            echo 'yes'
            ${cmd__run}
            bcftools merge -file-list ${study_vcf_files} -Ou | bcftools sort -T ./bcftools_tmp -Oz -o pre_${mode}_${mode2}_\${vcf_name}__vcf.vcf.gz
            bcftools index pre_${mode}_${mode2}_\${vcf_name}__vcf.vcf.gz
            ${cmd}
        else
          echo 'no'
          bcftools sort ${study_vcf_files} -T ./bcftools_tmp -Oz -o ${mode}_${mode2}_\${vcf_name}_out.vcf.gz
