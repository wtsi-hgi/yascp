...
process cellbender__remove_background_test {
  // Remove ambient RNA
  // ------------------------------------------------------------------------
  //cache false    // cache results from run
  //maxForks 2   // hard to control memory usage. limit to 3 concurrent
// cb_plot_input,out_paths,results_list,experimentid_outdir_cellbenderunfiltered_expectedcells_totaldropletsinclude
  ...
  script:
    ...
    try {
      test_sample_change = params.cellbender[experiment_id].value
    }catch(Exception ex) {
      test_sample_change = 'none'
    }
     try {
        test_sample_change2 = params.cellbender["$experiment_id"].value
    }catch(Exception ex) {
      test_sample_change2 = 'none'
    }
     try {
      test_sample_change3 = params.cellbender["idv"].value
    }catch(Exception ex) {
      test_sample_change3 = 'none'
    }
    outdir = "${outdir_prev}/${experiment_id}"
    lr_string = "${learning_rate}".replaceAll("\\.", "pt")
    lr_string = "${lr_string}".replaceAll("-", "neg")
    ...
    """
      echo ${test_sample_change}
      echo ${test_sample_change2}
      echo ${test_sample_change3}
      
      echo cellbender remove-background --input txd_input ${gpu_text_info} --output ${outfile} --expected-cells \$(cat ${expected_cells}) --total-droplets-included \$(cat ${total_droplets_include}) --model full --z-dim ${zdims} --z-layers ${zlayers} --low-count-threshold ${low_count_threshold} --epochs ${epochs} --learning-rate ${learning_rate} --fpr ${fpr}
    """
}
